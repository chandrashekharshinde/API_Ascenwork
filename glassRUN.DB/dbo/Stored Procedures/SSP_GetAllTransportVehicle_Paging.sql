...
			@Field10Criteria=tmp.[Field10Criteria],
		...
		IF(RTRIM(@orderBy) = '') BEGIN SET @orderBy = 'TransportVehicleId' END
		...
		Set @sql='
				...
				SELECT CAST((
					...
					From (
						SELECT 
							ROW_NUMBER() OVER (ORDER BY   ISNULL(tv.UpdatedDate, tv.CreatedDate) desc) as RowNumber, 
							COUNT(tv.[TransportVehicleId]) OVER () as TotalCount,
							...
							tv.[SequenceNumber],
							tv.[IsVehicleInsured],
							tv.[InsuranceValidityDate],
							tv.[IsFitnessCertificateAvailed],
							tv.[FitnessCertificateDate],
							tv.[VehicleOwnerName],
						...
						Left Join [dbo].[Company] c on tv.TransporterId=c.CompanyId And c.CompanyType=28 And c.IsActive=1  --Transporter==>28(Lookup)
						Left Join [dbo].[LookUp] ltv on tv.VehicleTypeId=ltv.LookUpId And ltv.LookupCategory=8 And ltv.IsActive=1 --VehicleType==>8(LookupCategory)
						Left Join [dbo].[TruckSize] ts on tv.TruckSizeId=ts.TruckSizeId And ts.IsActive=1
						WHERE --tv.IsActive = 1 and 
						' + @whereClause +'
						) as tmp 
