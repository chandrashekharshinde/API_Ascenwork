Create  PROCEDURE [dbo].[SSP_AppFormsList_SelectByCriteria] --'SupplierLOBId = 2 AND  Version <> '''' AND roleMasterid  IN (SELECT RoleId FROM  dbo.UserDetails WHERE UserId=39)'
...
ORDER BY ' + @SortExpression +'
 FOR XML PATH(''AppFormsList''),ELEMENTS)AS XML))'
