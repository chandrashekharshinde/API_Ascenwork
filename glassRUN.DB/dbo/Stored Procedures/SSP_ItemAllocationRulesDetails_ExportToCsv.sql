Create PROCEDURE [dbo].[SSP_ItemAllocationRulesDetails_ExportToCsv] --'<Json><ServicesAction>LoadItemAllocationRulesDetailsList</ServicesAction><PageIndex>0</PageIndex><PageSize>20</PageSize><OrderBy></OrderBy><OrderByCriteria></OrderByCriteria><CompanyType></CompanyType><CompanyTypeCriteria></CompanyTypeCriteria><SKUCode></SKUCode><SKUCodeCriteria></SKUCodeCriteria><Company></Company><CompanyCriteria></CompanyCriteria><Description></Description><DescriptionCriteria></DescriptionCriteria><RemainingQuantity></RemainingQuantity><RemainingQuantityCriteria></RemainingQuantityCriteria><AllocatedValue></AllocatedValue><AllocatedValueCriteria></AllocatedValueCriteria><FromDate>07/01/2019</FromDate><FromDateCriteria>=</FromDateCriteria><ToDate></ToDate><ToDateCriteria></ToDateCriteria></Json>'
(
@xmlDoc XML
)
AS
...
,[RuleId]
  ...
  FROM [dbo].[Rules] where IsActive=1 and RuleType=2)
	  as Rules where IsActive=1 and RuleType=2 and  ' + @whereClause +') as tmp
