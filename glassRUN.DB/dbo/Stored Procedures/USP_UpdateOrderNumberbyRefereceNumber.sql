  ...
  BEGIN TRY
   ...
   select * into #tmpOrder
    FROM OPENXML(@intpointer,'Json',2)
		...
		[PuchaseOrderNumber] nvarchar(150)
		...
		DECLARE @purchaseOrderNumber nvarchar(200)
	
       SET @orderId=(SELECT #tmpOrder.OrderId FROM #tmpOrder)
		  SET @OrderNumber=(SELECT #tmpOrder.OrderNumber FROM #tmpOrder)
		   SET @purchaseOrderNumber=(SELECT #tmpOrder.[PuchaseOrderNumber] FROM #tmpOrder)
		
		UPDATE dbo.[Order] SET OrderNumber=@OrderNumber,SalesOrderNumber=@OrderNumber ,PurchaseOrderNumber=@purchaseOrderNumber,CurrentState=3  WHERE OrderId=@orderId
