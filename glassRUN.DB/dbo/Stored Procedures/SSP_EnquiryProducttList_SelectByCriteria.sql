CREATE PROCEDURE [dbo].[SSP_EnquiryProducttList_SelectByCriteria]--'ORDERID IN (SELECT ORDERID FROM ORDERMOVEMENT WHERE [DeliveryPersonnelId] = 19 AND ActualTimeOfAction IS  NULL)','',dfdf
@WhereExpression nvarchar(2000) = '1=1',
@SortExpression nvarchar(2000) = 'op.EnquiryProductId',
...
IF(RTRIM(@SortExpression) = '') BEGIN SET @SortExpression = 'op.EnquiryProductId' END
...
SET @sql = '(SELECT CAST((SELECT ''true'' AS [@json:Array] ,
 ...
 FROM EnquiryProduct op join Enquiry o on o.EnquiryId=op.EnquiryId
   WHERE   op.IsActive = 1  and op.ProductCode !=''55909001'' and ' + @WhereExpression + '
