...
CREATE PROCEDURE [dbo].[SSP_CompanyDetailList_RoleParent] --'<Json><ServicesAction>LoadAllCompanyDetailListByDropDown</ServicesAction><RoleMasterId>14</RoleMasterId></Json>'
(
@xmlDoc XML
)
AS
...
;WITH XMLNAMESPACES('http://james.newtonking.com/projects/json' AS json)  select cast ((  select distinct 'true' AS [@json:Array]  
  ...
  from  Company c   Where c.CompanyType in (
SELECT ID FROM [dbo].[fnGetCompanyTypeParentId] (@CompanyTypeId))
