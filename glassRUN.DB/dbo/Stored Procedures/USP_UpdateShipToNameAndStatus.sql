...
BEGIN 
	...
	BEGIN TRY
			...
			SELECT * INTO #tmpShipTo
			FROM OPENXML(@intpointer,'Json/ShipToList',2)
			 WITH
             (
		   ...
		   SET DeliveryLocation.DisplayName = #tmpShipTo.DisplayName, DeliveryLocation.IsActive = #tmpShipTo.IsActive,
					DeliveryLocation.ModifiedDate = GETDATE(),DeliveryLocation.ModifiedBy = #tmpShipTo.UserId
		   FROM #tmpShipTo WHERE dbo.DeliveryLocation.DeliveryLocationId = #tmpShipTo.DeliveryLocationId 
				
        SELECT 1 as DeliveryLocationId FOR XML RAW('Json'),ELEMENTS
