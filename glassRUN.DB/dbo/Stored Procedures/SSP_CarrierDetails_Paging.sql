...
SET @orderBy = 'Convert(date,PraposedTimeOfAction) desc'
 ...
 ISNULL(DeliveryPersonnelId,0) as DeliveryPersonnelId,
  ...
  ExpectedShift,
  CONVERT(varchar(11),ExpectedTimeOfAction,103) as ExpectedTimeOfAction,
  ...
  CompanyName,CONVERT(varchar(11),tmp.ExpectedTimeOfDelivery,103) as ExpectedTimeOfDelivery, 
   (SELECT [dbo].[fn_GetTotalRecivingCapacityPalettes] (ExpectedTimeOfDelivery,ShipTo,SoldTo,ISNULL(CONVERT(bigint,Capacity),0))) as ReceivedCapacityPalettes,
	 ...
	 [PreviousState]     
      ,[TruckSizeId], 
	  [TruckSize] from 
	  (SELECT ROW_NUMBER() OVER (ORDER BY ISNULL(om.ExpectedTimeOfAction,ISNULL(om.PraposedTimeOfAction,ISNULL(o.ModifiedDate,o.CreatedDate))) desc) as rownumber , COUNT(*) OVER () as TotalCount,o.OrderId,o.OrderType,
  o.OrderNumber, 
...
(case when ExpectedShift IS NOT NULL then ExpectedShift ELse PraposedShift END) as ExpectedShift,
 (case when ExpectedTimeOfAction IS NOT NULL then ExpectedTimeOfAction ELse PraposedTimeOfAction END) as ExpectedTimeOfAction,
  ...
  [SecondaryAddress] ,   
	 ...
	 o.StockLocationId as BranchPlantName,''false'' as TruckInPlateNumberCheck,''true'' as TruckOutPlateNumberCheck,
	  ...
	  --(SELECT TOP 1 PlateNumber From (select Top 1 * from OrderLogistichistory where Orderid=o.OrderId and PlateNumberBy = ''Carrier''   ORDER BY OrderLogistichistoryId DESC) x ORDER BY OrderLogistichistoryId) as PreviousPlateNumber,
...
(select top 1 ActualEmpties from Company where companyid = o.SoldTo) as ActualEmpties,'
  ...
  FOR XML path(''OrderProductList''),ELEMENTS) AS xml)) AS enquiryProduct  
   ...
   WHERE o.IsActive = 1 and o.OrderType not in (''SG'',''S5'',''S6'') and o.SalesOrderNumber is not null and  ' + @whereClause + 
     ') as tmp where 
