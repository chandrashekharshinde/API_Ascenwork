...
set @sql=
			...
			Code
			from (
			SELECT  ROW_NUMBER() OVER (ORDER BY LookUpId desc) as rownumber , COUNT(*) OVER () as TotalCount
			,[LookUpId]
		,[LookupCategory] as LookupCategoryId
		,(Select Name from LookupCategory where LookupCategory.LookUpCategoryId= Lookup.LookupCategory) as LookupCategory
		,[Name]
		,[Code]
      
      
			FROM [dbo].[Lookup] where IsActive = 1
