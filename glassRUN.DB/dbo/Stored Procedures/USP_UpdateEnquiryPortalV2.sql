CREATE PROCEDURE [dbo].[USP_UpdateEnquiryPortalV2]
...
Declare @LocationId bigint
	...
	Declare @MainEnquiryGuid nvarchar(200)=''
	...
	SELECT
		...
		@tmpCollectionCode=tmp.[CollectionLocationCode],
		@CopyEnquiryGuid=tmp.[CopyEnquiryGuid],
		@MainEnquiryGuid=tmp.[EnquiryGuid]
        ...
        (
			...
			[CollectionLocationCode] nvarchar(100),
			[ShipTo] BIGINT,
			[SoldTo] BIGINT,
			[TruckSizeId] BIGINT
        )tmp
   ...
   Select * into #tmpPickDate  FROM OPENXML(@intpointer,'Json',2)
		...
		Print @currentState
			...
			FROM OPENXML(@intpointer,'Json/ProductList',2)
			 WITH
             (
			...
			[CurrentItemTruckCapacityFullInTon] decimal(10,2),
	  ...
	  END
          ...
          SELECT
			...
			dbo.EnquiryProduct.[TotalWeight] = #tmpEnquiryProduct.[CurrentItemTruckCapacityFullInTon],
			dbo.EnquiryProduct.[UOM] = #tmpEnquiryProduct.[UOM],
			dbo.EnquiryProduct.[EnquiryAutoNumber] = (Select EnquiryAutoNumber from Enquiry where EnquiryId=@EnquiryId )
		...
		--dbo.EnquiryProduct.EnquiryProductId=#tmpEnquiryProduct.EnquiryProductId
          ...
          SELECT
			...
			#tmpEnquiryProduct.[CollectionLocationCode],
        	1,
        	GETDATE(),        	
			...
			#tmpEnquiryProduct.[CurrentItemTruckCapacityFullInTon],
			#tmpEnquiryProduct.[PackingItemCount],
			#tmpEnquiryProduct.[PackingItemCode],
			#tmpEnquiryProduct.IsPackingItem,
			#tmpEnquiryProduct.[UOM],
			(Select EnquiryAutoNumber from Enquiry where EnquiryId=@EnquiryId )
