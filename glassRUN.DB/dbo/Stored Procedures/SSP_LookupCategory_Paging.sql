...
DECLARE @LookupCategoryId bigint
...
SELECT 
	...
	@LookupCategoryId = tmp.[LookupCategoryId],
    @Remarks = tmp.[Remarks],
...
set @sql=
			...
			TotalCount,
			LookupCategoryId,
			Name,Remarks, CreatedDate,ModifiedDate
			from (
			SELECT ROW_NUMBER() OVER (ORDER BY LookUpCategoryId desc) as rownumber , COUNT(*) OVER () as TotalCount ,
			LookUpCategoryId,[Name], isnull(Remarks,'''') as Remarks, CreatedDate, isnull(ModifiedDate,''1900-01-01'') as ModifiedDate
			FROM [dbo].[LookupCategory] where IsActive = 1
